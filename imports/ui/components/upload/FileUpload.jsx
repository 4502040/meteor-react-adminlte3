...
class FileUploadComponent extends Component {
  ...
  uploadIt(e) {
    ...
    if (e.currentTarget.files && e.currentTarget.files[0]) {
      ...
      if (file) {
        let uploadInstance = Poster.insert({
          file: file,
          meta: {
  ...
  render() {
    ...
    if (this.props.files && this.props.docsReadyYet) {
      ...
      let display = fileCursors.map((aFile, key) => {
        ...
        let link = Poster.findOne({_id: aFile._id}).link('thumbnail');  //The "view/download" link

        link = `${link}?${new Date().getTime()}`;

        let origFileUrl = Poster.findOne({_id: aFile._id}).link(); 

        origFileUrl = `${origFileUrl}?${new Date().getTime()}`;
...
export default withTracker( ( props ) => {
  const filesHandle = Meteor.subscribe('poster.postId', props.postId);
  const docsReadyYet = filesHandle.ready();
  const files = Poster.find({}, {sort: {name: 1}}).fetch();
