...
class ImageUploaderComponent extends Component {
  ...
  uploadIt(e) {
    ...
    if (e.currentTarget.files && e.currentTarget.files[0]) {
      ...
      if (file) {
        let uploadInstance = Image.insert({
          file: file,
          meta: {
  ...
  render() {
    ...
    if (this.props.files && this.props.docsReadyYet) {
      ...
      let display = fileCursors.map((aFile, key) => {
        ...
        let link = Image.findOne({_id: aFile._id}).link('thumbnail');  //The "view/download" link

        link = `${link}?${new Date().getTime()}`;

        let origFileUrl = Image.findOne({_id: aFile._id}).link(); 
...
export default withTracker( ( props ) => {
  const filesHandle = Meteor.subscribe('image.postId', props.postId);
  const docsReadyYet = filesHandle.ready();
  const files = Image.find({}, {sort: {name: 1}}).fetch();
